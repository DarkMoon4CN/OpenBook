...
namespace Log4Simple.Util
{
    ...
    public sealed class LogLog
    {
        ...
        /// <summary>
        /// delete file when system start
        /// </summary>
        private static void ifDeleteFile()
        {
          
            if (File.Exists(s_Log4Simple_Log_File))
                File.Delete(s_Log4Simple_Log_File);

                
        }

        /// <summary>
        /// write info to local file
        /// </summary>
        /// <param name="message"></param>
        private static void Log2File(string message)
        {

            using (var sw = new StreamWriter(s_Log4Simple_Log_File, (File.Exists(s_Log4Simple_Log_File) ? true : false), Encoding.Default))
            {
                sw.Write(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss ffff ") + message + "\r\n");
                sw.Flush();
                sw.Close();
            }
        }
        ...
        private static string s_Log4Simple_Log_File = SystemInfo.ConvertToFullPath("Log4Simple.log", true);
        /// <summary>
        ///  Default debug level
        /// </summary>
