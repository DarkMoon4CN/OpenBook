...
namespace Log4Simple.Appender
{
    ...
    public abstract class BufferingAppenderSkeleton : AppenderSkeleton
    {
                    ...
                    if (m_lossy)
                    {
                        ...
                        if (flushLossyBuffer)
                        {
                            if (m_lossyEvaluator != null)
                            {
                                ...
                                LoggingData[] bufferedEvents = m_cb.PopAll();
                                ArrayList filteredEvents = new ArrayList(bufferedEvents.Length);

                                foreach (LoggingData loggingEvent in bufferedEvents)
                                {
                                    if (m_lossyEvaluator.IsTriggeringEvent(loggingEvent,null))
                                    {
                                ...
                                {
                                    SendBuffer((LoggingData[])filteredEvents.ToArray(typeof(LoggingData)));
                                }
                            }
                            else
                            {
        ...
        /// </para>
        /// </remarks>
        override protected void Append(LoggingData loggingEvent)
        {
            m_LastAppendLoggingEventDt = DateTime.Now;
            ...
            lock (this)
            {
                ...
                if (m_cb == null || m_bufferSize <= 1)
                {
                    ...
                    if ((!m_lossy) ||
                        (m_evaluator != null && m_evaluator.IsTriggeringEvent(loggingEvent, bufferLen)) ||
                        (m_lossyEvaluator != null && m_lossyEvaluator.IsTriggeringEvent(loggingEvent, bufferLen)))
                    {
                        // Not buffering events, send immediately
                        SendBuffer(new LoggingData[] { loggingEvent });
                    }
                }
                else
                {
                    ...
                    LoggingData discardedLoggingEvent = m_cb.Append(loggingEvent);
                    ...
                    if (discardedLoggingEvent != null)
                    {
                        ...
                        else
                        {
                            ...
                            if (m_lossyEvaluator == null || !m_lossyEvaluator.IsTriggeringEvent(discardedLoggingEvent, null))
                            {
                            ...
                            if (m_evaluator != null && m_evaluator.IsTriggeringEvent(loggingEvent, m_cb))
                            {
                                SendFromBuffer(discardedLoggingEvent, m_cb);
                            }
                            else if (discardedLoggingEvent != null)
                            {
                                ...
                                SendBuffer(new LoggingData[] { discardedLoggingEvent });
                            }
                        }
                    }
                    else
        ...
        /// </para>
        /// </remarks>
        virtual protected void SendFromBuffer(LoggingData firstLoggingEvent, CyclicBuffer buffer)
        {
            LoggingData[] bufferEvents = buffer.PopAll();
            ...
            {
                SendBuffer(new LoggingData[] { firstLoggingEvent });
            }
            else
            {
                // Create new array with the firstLoggingEvent at the head
                LoggingData[] events = new LoggingData[bufferEvents.Length + 1];
        ...
        /// </para>
        /// </remarks>
        abstract protected void SendBuffer(LoggingData[] events);
        ...
        /// 最近一次记录日志时间
        /// </summary>
        protected DateTime m_LastAppendLoggingEventDt = DateTime.Now;
