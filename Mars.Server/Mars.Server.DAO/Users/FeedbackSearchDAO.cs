...
{
    public class FeedbackSearchDAO
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        public bool Delete(int feedbackID)
        {
            ...
            strSql.AppendFormat(@"DELETE FROM M_User_Feedback WHERE FeedbackID = {0}"
                , feedbackID.ToString());
        ...
        /// <param name="sp"></param>
        /// <returns></returns>
        protected virtual List<SqlParameter> ParseToSqlParameters(FeedbackSearchEntity sp)
        {
        ...
        /// <param name="sp"></param>
        /// <returns></returns>
        protected virtual SqlParameter CreateParameter_Table(FeedbackSearchEntity sp)
        {
            StringBuilder sbtable = new StringBuilder();
            sbtable.Append(@"dbo.M_User_Feedback AS f
                LEFT JOIN dbo.M_User AS u ON u.UserID = f.UserID");

            return new SqlParameter("@TableName", sbtable.ToString());
        }

        protected virtual SqlParameter CreateParamter_Orderby(FeedbackSearchEntity sp)
        {
            StringBuilder sb = new StringBuilder();

            sb.Append(" f.CreateTime DESC ");
        ...
        /// <param name="sp"></param>
        /// <returns></returns>
        protected virtual SqlParameter CreateParameter_Fileds(FeedbackSearchEntity sp)
        {
            StringBuilder sbfileds = new StringBuilder();
            sbfileds.Append(@"f.*
                ,CONVERT(varchar(30), f.CreateTime, 20) as FormatCreateTime
                ,ISNULL(u.LoginName,'未知用户') AS UserName");
        ...
        /// <param name="sp"></param>
        /// <returns></returns>
        protected virtual SqlParameter CreateParameter_Where(FeedbackSearchEntity sp)
        {
            StringBuilder sbwhere = new StringBuilder(" 1=1 ");

            if (!string.IsNullOrEmpty(sp.StartTime) && !string.IsNullOrEmpty(sp.EndTime))
            {
                sbwhere.AppendFormat(" AND f.CreateTime BETWEEN '{0} 00:00:00' AND '{1} 23:59:59' "
                    , sp.StartTime, sp.EndTime);
            }
            else if (string.IsNullOrEmpty(sp.StartTime) && !string.IsNullOrEmpty(sp.EndTime))
            {
                sbwhere.AppendFormat(" AND f.CreateTime < '{0} 23:59:59' "
                   , sp.EndTime);
            }
            else if (!string.IsNullOrEmpty(sp.StartTime) && string.IsNullOrEmpty(sp.EndTime))
            {
                sbwhere.AppendFormat(" AND f.CreateTime > '{0} 00:00:00' "
                    , sp.StartTime);
            }
